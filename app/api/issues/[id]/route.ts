...
export async function GET(request: Request, { params }: { params: { id: string } }) {
  try {
    ...
    const issue = await Issue.findById(params.id).populate("createdBy", "name email")
...
export async function PATCH(request: Request, { params }: { params: { id: string } }) {
  try {
    ...
    if (issue.status !== "Pending") {
      return NextResponse.json({ error: "Cannot update issue that is not in Pending status" }, { status: 400 })
    }
...
export async function DELETE(request: Request, { params }: { params: { id: string } }) {
  try {
    ...
    // Check if issue is still in Pending status
    if (issue.status !== "Pending") {
      return NextResponse.json({ error: "Cannot delete issue that is not in Pending status" }, { status: 400 })
    }
